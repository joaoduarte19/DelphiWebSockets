...
begin
  ...
  try
    ...
    if WSResourceName.StartsWith('/') then
      LWSResourceName := WSResourceName.Substring(1)
    else
...
// Timeout not supported on Android
    try
      ...
      begin
        var
        LLine := Format('WriteTimeout not supported. Type: %s, error: %s',
...
begin
  {$IF DEFINED(DEBUG_WS)}
  var
  LLine := Format('AType: %s, AMsg: %s', [AType, AMsg]);
